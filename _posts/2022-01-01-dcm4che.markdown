...
期待通りの文字列として扱うには、`dcm4chee-arc-light`に`DICOMファイル`を入れる前に加工する必要あり。  
...
ちなみに `/subsystem=datasources:read-resource` で登録済の`data-source`一覧が得られる。
...
|Apache DS 2.0.0-M24|可|
|Apache DS 2.0.0-M9|可|
